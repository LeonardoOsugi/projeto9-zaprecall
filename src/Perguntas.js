...
export default function Perguntas({setCount}) {
    ...
    return (
        <>
            {perguntas.map((p, i) =>
                <Cards key={i} index={i} flashcards={p.flashcard} perguntas={p.pergunta} respostas={p.resposta} setCount={setCount} status={status} setStatus={setStatus}/>
            )}
        </>
    )
}